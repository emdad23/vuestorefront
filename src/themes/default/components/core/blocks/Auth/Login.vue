...
<div>
    <header class="modal-header py25 px65 h1 serif weight-700 bg-cl-secondary">
        {{ $t('Log in') }}
        <i slot="close" class="modal-close material-icons cl-bg-tertiary" @click="close">
            close
        </i>
    </header>
    ...
    <div class="modal-content bg-cl-primary pt30 pb60 px65 cl-secondary">
        <form @submit.prevent="login" novalidate>
            <base-input class="mb10" type="email" name="email" focus v-model="email" @blur="$v.email.$touch()" :placeholder="$t('E-mail address *')" :validations="[
            {
            ...
            <base-input class="mb10" type="password" name="password" v-model="password" @blur="$v.password.$touch()" :placeholder="$t('Password *')" :validations="[{
            condition: !$v.password.required && $v.password.$error,
            text: $t('Field is required.')
          }]" />
            <div class="row">
                <base-checkbox class="col-xs-7 col-sm-6 mb20" id="remember" v-model="remember">
                    {{ $t('Remember me') }}
                </base-checkbox>
